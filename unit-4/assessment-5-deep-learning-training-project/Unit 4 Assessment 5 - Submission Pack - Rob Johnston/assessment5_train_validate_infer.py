...
- 3.1 two ways to train a deep learning model: Iris data and a
  Trade-A-Them-style synthetic cryptocurrency signal dataset
- 3.2 validation-set hyperparameter tuning using the Iris data
- 3.3 inference using the trained Iris model

Trade-A-Them is used as the project context because it is Rob Johnston's
active cryptocurrency trading-bot project. The synthetic dataset is deliberately
generated locally so no exchange credentials, live account values or private
trading records are exposed in the college evidence.
"""
...
def tradeathem_synthetic_arrays() -> tuple[np.ndarray, np.ndarray, list[str]]:
    """Create safe synthetic crypto-signal features for the Trade-A-Them context."""
    ...
    return x.astype(np.float32), y.astype(np.int64), ["risk_exit", "hold", "buy_signal"]
...
def save_loss_plot(rows: list[dict], dataset: str, output_path: Path):
    ...
    plt.ylabel("Loss")
    display_name = {
        "iris": "Iris baseline dataset",
        "tradeathem_synthetic_crypto": "Trade-A-Them-style synthetic crypto signal dataset",
    }.get(dataset, dataset)
    plt.title(f"Assessment 5 {display_name} training")
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    synthetic_x, synthetic_y, synthetic_names = tradeathem_synthetic_arrays()
    ...
    synthetic_model, synthetic_metadata, synthetic_best, _, synthetic_rows, _ = train_dataset(
        "tradeathem_synthetic_crypto", synthetic_x, synthetic_y, synthetic_names, synthetic_configs, device
    )
    ...
    synthetic_checkpoint = OUTPUTS / "assessment5_tradeathem_synthetic_crypto_model_checkpoint.pt"
    torch.save(
        {
    ...
    save_loss_plot(
        all_rows,
        "tradeathem_synthetic_crypto",
        OUTPUTS / "assessment5_tradeathem_synthetic_crypto_training_loss.png",
    )
    ...
    summary = {
        ...
        "training_demonstrations": {
            ...
            "tradeathem_synthetic_crypto": {
                ...
                "context": "Synthetic crypto-signal dataset used to represent a safe Trade-A-Them-style classification workflow without exposing private trading data.",
            },
        },
        ...
        "outputs": [
            ...
            "assessment5_tradeathem_synthetic_crypto_model_checkpoint.pt",
            "assessment5_training_metrics.csv",
            "assessment5_summary.json",
            "assessment5_iris_training_loss.png",
            "assessment5_tradeathem_synthetic_crypto_training_loss.png",
